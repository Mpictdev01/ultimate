...
export default function EditGalleryForm({ item }: { item: any }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await updateGalleryItem(item.id, data);

    if (result.success) {
      router.push("/admin/gallery");
    } else {
      setError(result.error || "Failed to update gallery item");
      setLoading(false);
    }
  };
    ...
    <div className="admin-page">
      ...
      <div className="admin-card">
        ...
        <form onSubmit={handleSubmit} className="admin-form">
          ...
          <ImageUpload name="image_url" label="Gallery Image" defaultValue={item.image_url} />

          <div className="admin-form__group">
            <label className="admin-form__label">Alt Text / Description</label>
            <input name="alt_text" defaultValue={item.alt_text} className="admin-form__input" placeholder="Description of the image" />
          </div>
